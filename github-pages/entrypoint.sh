...
read -a INPUT_DECISIONSPICS <<< "${INPUT_DECISIONSPICS}"
python /hoi4-icon-search-action/github-pages/hoi4_icon_search_gen.py --goals ${INPUT_GOALS[@]//\"/} --ideas ${INPUT_IDEAS[@]//\"/} --texticons ${INPUT_TEXTICONS[@]//\"/} --events ${INPUT_EVENTS[@]//\"/} --news-events ${INPUT_NEWSEVENTS[@]//\"/} --agencies ${INPUT_AGENCIES[@]//\"/}  --decisions ${INPUT_DECISIONS[@]//\"/} --decisions-cat ${INPUT_DECISIONSCAT[@]//\"/} --decisions-pics ${INPUT_DECISIONSPICS[@]//\"/} --title "${INPUT_TITLE}" --favicon "${INPUT_FAVICON}" --modified-images-str "${INPUT_MODIFIEDFILES}" --template-path "/hoi4-icon-search-action/github-pages/index.template"
touch .nojekyll
